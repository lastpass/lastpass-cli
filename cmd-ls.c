...
static bool json_listing = false;
	...
	if (json_listing){
		if (node->name) {
			if (node->account) {
				...
				terminal_printf(",{\"site\":\"%s}", buf.bytes);
				nodesPrinted = nodesPrinted+1;
				free(buf.bytes);
			}
			else if (node->shared && !firstPrinted && level==0) {
				terminal_printf("{\"vault\":[{\"folder\":[{\"name\":\"" TERMINAL_FG_CYAN TERMINAL_BOLD "%s" TERMINAL_RESET "\"}", node->name);
				firstPrinted = true;
			}
			else if (node->shared && firstPrinted && level==0) {
				terminal_printf("]},{\"folder\":[{\"name\":\"" TERMINAL_FG_CYAN TERMINAL_BOLD "%s" TERMINAL_RESET "\"}", node->name);
			}
			else if (!node->shared && !node->account && !firstPrinted && level==0) {
				terminal_printf("{\"vault\":[{\"folder\":[{\"name\":\"" TERMINAL_FG_BLUE TERMINAL_BOLD "%s" TERMINAL_RESET "\"}", node->name);
				firstPrinted = true;
			}
			else if (!node->shared && !node->account && !firstPrinted && level==0) {
				terminal_printf("]},{\"folder\":[{\"name\":\"" TERMINAL_FG_BLUE TERMINAL_BOLD "%s" TERMINAL_RESET "\"}", node->name);
				firstPrinted = true;
			}
		}
...
int cmd_ls(int argc, char **argv)
{
    ...
    while ((option = getopt_long(argc, argv, "lmuj", long_options, &option_index)) != -1) {
		switch (option) {
			case 'S':
